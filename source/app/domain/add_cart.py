...
def add_to_cart(username, product_id, quantity, repos: Repositories):
    ...
    try:
        product = (
            repos.db.session.query(product_repo.model)
            .filter(product_repo.model.id == product_id)
            .first()
        )
        if not product.stock > quantity:
            raise NotEnoughStock(product.name, product.stock)
        user_id = (
            repos.db.session.query(user_repo.model.id)
            .filter(user_repo.model.email == username)
            .scalar()
        )
        cart = (
            repos.db.session.query(cart_repo.model)
            .filter(cart_repo.model.user_id == user_id)
            .first()
        )
        if cart:
        ...
        cart_item = repos.cart_item.create_model_dto(
            product_id=product_id, quantity=quantity, cart_id=cart_obj.cart_id
        )
